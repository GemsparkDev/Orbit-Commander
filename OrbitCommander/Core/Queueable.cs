...
public abstract class Queueable
{
    ...
    {
        _logger.Try(delegate { Cost = int.Parse(_data[1]); }, 0);
        Name = _name;
        MaxCost = _maxCost;
        Texture = _texture;
    }
...
public class RepairQueue : Queueable
{
    ...
    {
        _logger.Try(delegate { module = UI.MissionSelectSlots[int.Parse(_data[2])]; }, 2);
    }
    protected override void Construct()
    {
...
public class SmeltQueue : Queueable
{
    ...
    {
        _logger.Try(delegate { pickup = UI.MissionSelectSlots[int.Parse(_data[2])]; }, 2);
    }
    protected override void Construct()
    {
