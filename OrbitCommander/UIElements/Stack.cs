...
public class Stack<T> : FunctionalWidget where T : IData
{
    ...
    public override void Draw(SpriteBatch _spriteBatch, Vector2 _parentPositon, float _transparency, Vector2 _center)
    {
        ...
        {
            _spriteBatch.Draw(StackTexture, _parentPositon + Offset - _center + (stackOffset + stackDepth * i) * UIManager.UIScale, null, Color.White, 0, UIManager.DimsOf(StackTexture) / 2, UIManager.UIScale, 0, 0);
        }
    }
}
