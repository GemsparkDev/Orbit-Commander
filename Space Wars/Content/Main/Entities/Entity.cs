...
public class Entity : IMissionComponent
{
    ...
    public bool ChildEnemy { get {return GetComponent<ChildEnemyTag>().ChildEnemy;} set {GetComponent<ChildEnemyTag>().ChildEnemy = value;}}
    ...
    public void Mine()
    {
        var comp = GetComponent<Mineable>();
        if(comp != null)
        {
            comp.Mine(MaxHealth);
        }
    ...
    {
        Entity entity = new Entity(_position, _velocity, _angle, 0);
        if(_hitSound == null)
        {
            _hitSound = Assets.Get(Sound.Hit);
        }
        entity.AddComponent(new Stealth(entity))
            .AddComponent(new Temp(entity))
        ...
        {
            damage = entity.StatusHolder.ModifyDamage(damage);
            if (damage > 0)
            {
                if(entity.Health > 0)
                {
                ...
                {
                    entity.Velocity += new Vector2(Util.OneToNegOne(), Util.OneToNegOne()) + Vector2.Normalize(entity.Position - Player.Position) * 0.3f * (float)damage;
                    entity.AngularVelocity += Util.OneToNegOne()*0.05f * (float)damage;
                }
            ...
            .AddComponent(new Cooldown(entity))
            .AddComponent(new ChildEnemyTag(entity))
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                if(Team == Team.Hostile)
                {
                    ...
                    {
                        Engine.SaveGame.CurrentMission.Add(new Missile() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                    else
                    {
                        Engine.SaveGame.CurrentMission.Add(new SummonShield() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                }
            }
            Velocity *= 0.8f;
    ...
    IEnumerable<int> OverloadBoss()
    {
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                if (Engine.SaveGame.GiveWeapon)
                {
                    Engine.SaveGame.CurrentMission.Add(new Shotgun() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
                else
                {
                    Engine.SaveGame.CurrentMission.Add(new SummonShield() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
            ...
            if (Health <= 0)
            {
                ...
                if (!hasExploded)
                {
                    ...
                    else
                    {
                        ...
                        if (Engine.SaveGame.GiveWeapon)
                        {
                            Engine.SaveGame.CurrentMission.Add(new LMG() { Position = this.Position, Velocity = normalizedAcceleration * 10, AngularVelocity = this.AngularVelocity });
                        }
                        else
                        {
                            Engine.SaveGame.CurrentMission.Add(new Reflective() { Position = this.Position, Velocity = normalizedAcceleration * 10, AngularVelocity = this.AngularVelocity });
                        }
                    }
                }
                else
    ...
    IEnumerable<int> ExcursionBoss()
    {
        ...
        while (true)
        {
            ...
            if (Health >= MaxHealth / 2 || CD[1] > 0)
            {
                ...
                if (nearestExpiredAlly.GetComponent<Health>() != null)
                {
            ...
            if (Health <= 0)
            {
                ...
                if (Engine.SaveGame.GiveWeapon)
                {
                    Engine.SaveGame.CurrentMission.Add(new Antimaterial() { Position = this.Position, Velocity = normalizedAcceleration * 10, AngularVelocity = this.AngularVelocity });
                }
                else
                {
                    Engine.SaveGame.CurrentMission.Add(new Nanomachines() { Position = this.Position, Velocity = normalizedAcceleration * 10, AngularVelocity = this.AngularVelocity });
                }
            }
            Velocity *= 0.8f;
            yield return 0;
            ...
            if (Health <= 0)
            {
                ...
                if (!isWeak)
                {
                    if (Engine.SaveGame.GiveWeapon)
                    {
                        ...
                        Engine.SaveGame.CurrentMission.Add(new Crossbow() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                    else
                    {
                        Engine.SaveGame.CurrentMission.Add(new PlasmaEngine() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                }
                else
                {
    ...
    IEnumerable<int> VeilBoss()
    {
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                if (Engine.SaveGame.GiveWeapon)
                {
                    Engine.SaveGame.CurrentMission.Add(new GrenadeLauncher() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
                else
                {
                    Engine.SaveGame.CurrentMission.Add(new StealthHull() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
            }
            yield return 0;
        }
    ...
    IEnumerable<int> InfernoBoss()
    {
        ...
        while (true)
        {
            ...
            if (Health <= 0 && flare.Health <= 0)
            {
                ...
                isExpired = true;
                SoundManager.PlaySound(Assets.Get(Sound.Death), Position);
                Engine.SaveGame.CurrentMission.Add(new Flamethrower() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
            }
            yield return 0;
        }
    }
    ...
    IEnumerable<int> FlareBoss(Entity _inferno)
    {
        ...
        while (true)
        {
            ...
            if (CD[1] <= 0)
            {
                ...
                else
                {
                    if (shotCount < 8)
                    {
                        ...
                        Engine.SaveGame.CurrentMission.Add(new FlameBolt(Position, Velocity + Util.ToUnitVector((float)(shotCount) * MathF.Tau / 8) * speed + new Vector2(Util.OneToNegOne(), Util.OneToNegOne()) / 2, Team, damage, 30, 1f));
                        SoundManager.PlaySound(Assets.Get(Sound.LMGFire), Position);
                        shotCount++;
                    }
                    else
            ...
            if (Health <= 0 && _inferno.Health <= 0)
            {
                ...
                isExpired = true;
                SoundManager.PlaySound(Assets.Get(Sound.Death), Position);
                Engine.SaveGame.CurrentMission.Add(new Fireball() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
            }
            yield return 0;
        }
    }
        ...
        while (true)
        {
            if (children.Count <= 0)
            {
                if (Health <= 0)
                {
                    ...
                    if (Engine.SaveGame.GiveWeapon)
                    {
                        Engine.SaveGame.CurrentMission.Add(new Spiral() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                    else
                    {
                        Engine.SaveGame.CurrentMission.Add(new CreateFighter() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                }
                else
                {
    ...
    IEnumerable<int> BloomBoss(List<Entity> segments, int damage)
    {
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                if (Engine.SaveGame.GiveWeapon)
                {
                    Engine.SaveGame.CurrentMission.Add(new SpewerModule() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
                else
                {
                    Engine.SaveGame.CurrentMission.Add(new Reflective() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
            }
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                if (!isWeak)
                {
                    if (Engine.SaveGame.GiveWeapon)
                    {
                        Engine.SaveGame.CurrentMission.Add(new GuidedRound() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                    else
                    {
                        Engine.SaveGame.CurrentMission.Add(new Decoy() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                }
                else
                {
    ...
    IEnumerable<int> StreamlineBoss(Entity _leftWing, Entity _rightWing)
    {
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                if (Engine.SaveGame.GiveWeapon)
                {
                    Engine.SaveGame.CurrentMission.Add(new SplitterModule() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
                else
                {
                    Engine.SaveGame.CurrentMission.Add(new Ablative() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
            }
            targetAngle = Util.ToAngle(Velocity);
    ...
    IEnumerable<int> DeadeyeBoss()
    {
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                if (Engine.SaveGame.GiveWeapon)
                {
                    Engine.SaveGame.CurrentMission.Add(new CrackShot() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
                else
                {
                    Engine.SaveGame.CurrentMission.Add(new Assault() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                }
            }
        ...
        while(true)
        {
            if (Health <= 0)
            {
                ...
                if(!isWeak)
                {
                    if (Engine.SaveGame.GiveWeapon)
                    {
                        Engine.SaveGame.CurrentMission.Add(new Fractal() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                    else
                    {
                        Engine.SaveGame.CurrentMission.Add(new Turtle() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
                    }
                }
            }
    ...
    IEnumerable<int> ClockworkBoss(Entity _cog)
    {
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                Engine.SaveGame.CurrentMission.Add(new OrionEngine() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 5, AngularVelocity = this.AngularVelocity });
            }
            targetAngle = Util.ToAngle(Velocity);
            RotateTowards(targetAngle);
            yield return 0;
    ...
    {
        var enemy = NewEnemy(position, Vector2.Zero, 8, 500, Assets.Get(Sprites.PickupDrone), Team.Friendly);
    ...
    IEnumerable<int> LargeMiner()
    {
        ...
        while (true)
        {
            ...
            if (Health <= 0)
            {
                ...
                Explode(10, ColliderRadius);
                SoundManager.PlaySound(Assets.Get(Sound.Explosion), Position);
                Engine.SaveGame.CurrentMission.Add(new SummonGrapplingHook() { Position = this.Position, Velocity = GetNormalizedAcceleration() * 10, AngularVelocity = this.AngularVelocity });
            }
            yield return 0;
        }
    }
    ...
    public static Entity NewProjectile(Texture2D _texture, Vector2 _position, Vector2 _velocity, float _angle, float _angularVelocity, Team _team, int _damage, int _stealth)
    {
        ...
        projectile.AddComponent(new ChildEnemyTag(projectile) { ChildEnemy = true });
        return projectile;
    }
    #region Projectiles
